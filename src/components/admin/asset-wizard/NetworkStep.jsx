...
// Solo props mínimos: formData, handleSelectChange, networkOptions
      ...
      <SelectContent>
        {networkOptions.map(net => (
          <SelectItem key={net.value} value={net.value}>
            {net.label} (chainId: {net.chainId})
          </SelectItem>
        ))}
      </SelectContent>
    </Select>
    {formData.chainId && (
      <p className="text-xs text-emerald-300/70">Chain ID seleccionada: {formData.chainId}</p>
    )}
  </div>
);
